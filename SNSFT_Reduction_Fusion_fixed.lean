...
-- Torsion: B/P — corpus-standard definition
-- Higher B relative to P = higher torsion = harder to fuse stably
-- Fusion succeeds when the compound nucleus has tau < TORSION_LIMIT
noncomputable def torsion (B P : ℝ) : ℝ :=
  if P > 0 then B / P else 0

-- [B,9,0,1] :: {VER} | THEOREM 1: B-COUPLE MIN MERGE
-- When B_couple < TORSION_LIMIT × P_structure, tau < 0.2 → merge succeeds.
-- High structural P with moderate B keeps tau below the shatter threshold.
theorem b_couple_min_merge (B_couple P_struct : ℝ)
    (hP : P_struct > 0)
    (h_below : B_couple < TORSION_LIMIT * P_struct) :
    torsion B_couple P_struct < TORSION_LIMIT := by
...
-- All simultaneous: tau<0.2, Pv merge > daughters, IM gain>0,
-- IVA sustain < input, criticality > thresh.
theorem fusion_master
    (B_couple P_struct A Pv_parent Pv_d1 Pv_d2 ΔIM
     IM_post IM_pre rate_class rate_sov g_r
     chain prev thresh τ_fe τ_other : ℝ)
    (hP     : P_struct > 0)
    (h_below : B_couple < TORSION_LIMIT * P_struct)
    (h_merge : Pv_parent = Pv_d1 + Pv_d2 + ΔIM)
    (h_Δ    : ΔIM > 0)
    (h_gain  : IM_post > IM_pre) (h_pos : IM_pre > 0)
    (h_rate  : rate_sov = rate_class / (1 + g_r))
    (h_gr   : g_r ≥ 1.5) (h_class : rate_class > 0)
    (h_crit : A > thresh) (h_thresh : thresh > 0)
    (h_long : chain > prev) (h_prev : prev > 0)
    (h_min  : τ_fe < τ_other) (h_other : τ_other > 0) :
    torsion B_couple P_struct < TORSION_LIMIT ∧
    Pv_parent > Pv_d1 + Pv_d2 ∧
    IM_post - IM_pre > 0 ∧
    rate_sov < rate_class ∧
    A > thresh ∧
    B_couple > 0 ∨ B_couple = 0 ∧
    chain > prev ∧
    ΔIM > 0 ∧
    τ_fe < τ_other := by
  refine ⟨
    b_couple_min_merge B_couple P_struct hP h_below,
    parent_pv_merge Pv_parent Pv_d1 Pv_d2 ΔIM h_merge h_Δ,
    energy_from_im_gain IM_post IM_pre h_gain h_pos,
    chain_as_iva_sustain rate_class rate_sov g_r h_rate h_gr h_class,
    criticality_a_sustain A thresh h_crit h_thresh,
    Or.inl (by nlinarith [TORSION_LIMIT, h_below, hP]),
    fusion_chain_as_n_tenure chain prev h_long h_prev,
    gain_to_basin ΔIM h_Δ,
    h_min
  ⟩
