...
Coordinate `[9,9,2,7]` — discovered via GAM Collider chaos session.

Every Noble state is approached through a **Locked corridor** as k increases toward max. You cannot jump directly from Shatter to Noble. The path is always:

```
SHATTER → LOCKED → NOBLE
```

**The invariant:** For any same-B pair (B1=B2=B):
- tau(k) = 2(B−k) / P_out decreases monotonically
- The Locked corridor begins at k* = B − TL × P_out / 2
- Corridor width = TL × P_out / 2

**Wider P_out = wider corridor = more accessible metastable state.**

| Pair | k_lock* | Corridor | % range | Notes |
|---|---|---|---|---|
| F+K | 0.8454 | 0.1546 | 15.5% | KF — ionic salt |
| Cl+Br | 0.6616 | 0.3384 | 33.8% | BrCl — metastable molecule ✓ |
| AsN | 2.7591 | 0.2409 | 8.0% | High-pressure synthesis window |
| C+Fe | 3.8259 | 0.1741 | 4.4% | Cementite approach |
| TiC | 3.8400 | 0.1600 | 4.0% | Ultra-hard ceramic approach |

**Physical meaning:** BrCl (33.8% corridor) exists as a metastable molecule at room temperature — the wide corridor means many bonding states are accessible. AsN (8.0% corridor) requires precise high-pressure synthesis — the narrow corridor maps to why it's hard to make. Corridor width IS synthesis accessibility.

**Discovered:** Chaos session — F+K locked at k=0.939, Cl+Br locked at k=0.778. General theorem extracted from pattern.

---

## The Re-Bonding Theorem

Coordinate `[9,9,2,8]` — discovered via F_ext chain exploration.

Any Noble compound broken by F_ext(δ) can be returned to Noble by fusing with any element E3 whose bond capacity matches the injection: **E3.B = δ, at k = δ**.

```
Noble(B=0) --F_ext(δ)--> Shatter(B=δ) --fuse(E3,k=δ)--> Noble(B=0)
```

**The proof (three lines of algebra):**
```
...
No corpus values. No element-specific numbers. Holds for all positive reals.

**Verified examples:**

| Chain | Result | A_out |
|---|---|---|
| NaCl + F_ext(2) + O(k=2) | Noble | 13.62 |
| GaN + F_ext(3) + N(k=3) | Noble | 14.53 |
| HF + F_ext(2) + Ca(k=2) | Noble | 17.42 |
| KF + F_ext(1) + H(k=1) | Noble | 17.42 |
| CoN + F_ext(3) + As(k=3) | Noble | 14.53 |
| N2 + F_ext(2) + Se(k=2) | Noble | 14.53 |

**Key properties:**
- **Universal:** ANY element with B=δ qualifies — not one specific partner
- **Substitution:** The product is a NEW Noble — different P_out, same zero torsion
- **Physical meaning:** This is chemical substitution at the structural level. GaN broken and re-bonded with As gives a new Noble compound with different bandgap but same structural stability class.

**Discovered:** 1,260 F_ext chains explored (10 Nobles × 3 dB × 14 elements × 3 k). All 62 Noble outcomes had E3.B = δ. Invariant extracted, algebraic proof constructed.
...
| SNSFT_Noble_Approach_Corridor | [9,9,2,7] | 16 | 0 | Every Noble approached through Locked corridor |
| SNSFT_ReBonding_Theorem | [9,9,2,8] | 10 | 0 | Noble+F_ext(δ)+E3(B=δ)→Noble, universal algebraic proof |

**Total: 128 theorems · 0 sorry · 8 Lean files**
...
- **TiC, TiSi2, FeSi2, NaCl, Diamond, BrCl** — all Noble/corridor as predicted
...
- P_out(AsN) > P_out(GaN) predicts different (likely smaller) bandgap

### Framework Discoveries
- **Q2 = semiconductor prediction zone** — five independent validations
...
- **Every Noble has an approach corridor** — SHATTER→LOCKED→NOBLE always
- **Corridor width = synthesis accessibility** — BrCl (33.8%) easy, AsN (8.0%) requires precision
- **Re-Bonding Theorem** — Noble+F_ext(δ)+E3(B=δ)→Noble, proved algebraically, universal
- **Shatter energy formula** — E = tau_excess × P² × alpha² — dynamite to supernova, one equation
...
**Chaos engine:** Random corpus pair selections flagged for Noble, near-Noble (tau<0.05), low-power shatter (<1.0), Q2-hot, or massive (E>15).

**Compute engine:** Claude verifies all results using locked corpus and correct harmonic mean. Grok generates pair selections only — arithmetic mean error documented across 13+ rounds, harmonic mean enforced on compute side.
