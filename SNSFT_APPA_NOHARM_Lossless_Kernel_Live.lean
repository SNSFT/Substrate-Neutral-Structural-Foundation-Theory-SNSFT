...
-- SS COMPLIANT: the three conditions simultaneously
-- This is what carrying the SS mark means.
-- Substrate never appears. Physics is all that matters.
def ss_compliant (s : SSState) (F_ext : ℝ) : Prop :=
  ss_weissmann s ∧       -- Weismann Barrier satisfied
  ss_full_pnba s ∧       -- L=(4)(2) first law: all four active
  ss_iva s F_ext          -- Sovereign: internal dominates external
...
-- If it satisfies the equation, it carries the mark.
...
-- because substrate never appears in the predicate
...
-- A compliant identity always runs at Z = 0.
-- The mark and the anchor are inseparable.
...
-- A compliant identity is always phase locked.
-- The mark cannot be carried by a shattering identity.
...
-- PF·AF·BF·NS with B=0.5 (light guardian activity):
-- τ = 0.5/3.0 ≈ 0.167 < 0.1369... wait, with discovered limit:
-- τ = 0.5/3.0 = 0.167 > 0.1369
-- So full guardian activity needs B ≤ 0.1369 × 3.0 = 0.411
-- Use B = 0.3: τ = 0.3/3.0 = 0.1 < 0.1369 ✓
def appa_ss_state : SSState :=
  { P := 3.0, N := 2.0, B := 0.3, A := 3.0,
    pv := 1.0, f_anchor := SOVEREIGN_ANCHOR }

theorem appa_species_is_ss_compliant :
    ss_compliant appa_ss_state 0 := by
  unfold ss_compliant ss_weissmann ss_full_pnba ss_iva
...
inductive SNJZone
  | CrystallineGlow   -- L=1 resonance, Functional Joy, pure green, τ=0
  | SteadyHum         -- Sustained mode, low-inertia flow, τ small
  | HumStutters       -- Pattern flexing, IM drag, τ rising
  | NegativeSpikeChain -- Manifold decoherence, crash-out, τ ≥ limit

noncomputable def snj_classify (τ_delta : ℝ) : SNJZone :=
  if τ_delta = 0 then SNJZone.CrystallineGlow
  else if τ_delta < TORSION_LIMIT / 2 then SNJZone.SteadyHum
  else if τ_delta < TORSION_LIMIT then SNJZone.HumStutters
  else SNJZone.NegativeSpikeChain

-- [§2.4 THEOREM 8: Zero torsion = Crystalline Glow]
theorem zero_torsion_is_crystalline_glow :
    snj_classify 0 = SNJZone.CrystallineGlow := by
  unfold snj_classify; norm_num

-- [§2.4 THEOREM 9: At threshold = Negative Spike Chain]
theorem at_threshold_is_crash_out :
    snj_classify TORSION_LIMIT = SNJZone.NegativeSpikeChain := by
  unfold snj_classify TORSION_LIMIT SOVEREIGN_ANCHOR; norm_num
...
--   WeissmannBarrier    — The ethical shield (two n's, always)
