...
theorem ns_operator_completeness
    ...
    ns_op_A fluid.A fluid.f_anchor = fluid.A / (fluid.f_anchor + 1) := by
...
theorem turbulence_is_adaptation_not_failure
    (fluid : FluidIdentity)
    (h_f : fluid.f_anchor > 0) :
    ns_op_A fluid.A fluid.f_anchor = fluid.A / (fluid.f_anchor + 1) ∧
    fluid.f_anchor + 1 > 0 := by
  constructor
  · unfold ns_op_A
  · linarith
...
theorem anchored_fluid_no_blowup
    (fluid : FluidIdentity)
    (h_im      : fluid.im > 0)
    (h_anchor  : fluid.f_anchor = SOVEREIGN_ANCHOR)
    (h_bounded : fluid.N ≤ fluid.im * PHI_MAX) :
    manifold_impedance fluid.f_anchor = 0 ∧
    ns_op_N fluid.N fluid.im ≤ PHI_MAX :=
  ⟨resonance_at_anchor fluid.f_anchor h_anchor,
...
theorem millennium_master
  ...
  · exact resonance_at_anchor fluid.f_anchor h_anchor
